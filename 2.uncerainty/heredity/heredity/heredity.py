...
def gene_num_probability(parent_gene, offer_gene):
    if offer_gene:
        if parent_gene == 0:
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    p = 1
    for person in names:
        tmp_gene , tmp_trait= conditions[person]["gene"], conditions[person]["trait"]
        p *= PROBS["trait"][tmp_gene][tmp_trait]
        if people[person]["mother"] == people[person]["father"] == None :
            p *= PROBS["gene"][tmp_gene]
        else:
            ...
            if tmp_gene == 0:
                p *= gene_num_probability(mum_gene, False) * gene_num_probability(dad_gene, False)
            elif tmp_gene == 1:
